...
public class MapChunkCache121_11 extends GenericMapChunkCache {
	...
	protected Supplier<GenericChunk> loadChunkAsync(DynmapChunk chunk) {
		CraftWorld cw = (CraftWorld) w;
		CompletableFuture<Optional<NBTTagCompound>> genericChunk = cw.getHandle().p().a.d(new ChunkCoordIntPair(chunk.x, chunk.z)); // WorldServer.getChunkSource().chunkMap.read(new ChunkCoordIntPair(chunk.x, chunk.z))
	...
	protected GenericChunk loadChunk(DynmapChunk chunk) {
		...
		try { // BUGBUG - convert this all to asyn properly, since now native async
			Optional<NBTTagCompound> opt = cw.getHandle()
					.p() // ServerLevel.getChunkSource
					.a // ServerChunkCache.chunkMap
					.d(cc) // SimpleRegionStorage.read(ChunkPos)
					.join();
			nbt = opt.orElse(null);
		} catch (CancellationException | NoSuchElementException ex) {
			// Replace 'Bukkit.getLogger()' with your preferred logger or handle as needed
			Bukkit.getLogger().log(Level.SEVERE, "Exception loading chunk " + chunk.x + "," + chunk.z, ex);
		}
		if (nbt != null) {
	...
	public int getFoliageColor(BiomeMap bm, int[] colormap, int x, int z) {
		return bm.<BiomeBase>getBiomeObject().map(BiomeBase::h).flatMap(BiomeFog::b).orElse(colormap[bm.biomeLookup()]); // BiomeBase::getSpecialEffects ; BiomeFog::getFoliageColorOverride
	}
	...
	public int getGrassColor(BiomeMap bm, int[] colormap, int x, int z) {
		...
		return fog.e().a(x, z, fog.d().orElse(colormap[bm.biomeLookup()])); // BiomeFog.getGrassColorModifier.modifyColor ; BiomeFog.getGrassColorOverride
	}
}
